# Storage Accounts (Overlay Remote) `[Microsoft.Storage/storageAccountsOverlayRemote]`
...
| Reference | Type |
| :-- | :-- |
| `br/artifacts:storage.storage-accounts:latest` | Remote reference |
...
```bicep
module storageAccountsOverlayRemote './storage/storage-accounts-overlay-remote/main.bicep' = {
  name: '${uniqueString(deployment().name, location)}-test-ssaorc'
  params: {
    // Required parameters
    name: '<<namePrefix>>ssaorc001'
    ...
    localUsers: [
      {
        ...
        storageAccountName: '<<namePrefix>>ssaorc001'
      }
    ]
    location: '<location>'
    lock: 'CanNotDelete'
